...
void MarkDistinct::restoreNextSpillPartition() {
  ...
  // The hash table will be rebuilt as we process the spilled input.
  processSpilledInput();
}
...
void MarkDistinct::recursiveSpillInput() {
  ...
  // Read all remaining data from the current partition and spill it.
  RowVectorPtr spilledInput;
  ...
  // Finish spilling the current partition and restore the next one.
  finishSpillInputAndRestoreNext();
}
...
void MarkDistinct::ensureInputFits(const RowVectorPtr& input) {
      ...
      currentUsage * spillConfig_->spillableReservationGrowthPct / 100);
  {
    ...
    if (pool()->maybeReserve(targetIncrementBytes)) {
      // If reservation triggered spilling, release the reservation.
      if (inputSpiller_ != nullptr) {
        pool()->release();
      }
      return;
...
SpillPartitionIdSet MarkDistinct::spillHashTable() {
  ...
  // table and rebuild it from spilled input during restore. No hash table
  // contents are written to disk. The returned partitionIdSet is only used
  // for assertion safety and code alignment with other spillable operators.
